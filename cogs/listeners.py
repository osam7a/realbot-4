...
class Listeners(commands.Cog):
  ...
  async def on_message_delete(self, message):
    ...
    with open("./dicts/deleted.json", "r") as f:
      load=json.load(f)
      load.append({"content":f"{message.content}", "author":f"{message.author}"})
  ...
  async def on_message_edit(self, before, after):
    ...
    with open("./dicts/edited.json", "r") as f:
      load=json.load(f)
      load.append({"author":f"{before.author}", "before":{"content":f"{before.content}"}, "after":{"content":f"{after.content}"}})        
